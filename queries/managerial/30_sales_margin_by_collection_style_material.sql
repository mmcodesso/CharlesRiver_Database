-- Teaching objective: Compare billed sales, credits, net sales, cost, and gross margin by product collection and lifecycle attributes.
-- Main tables: SalesInvoiceLine, ShipmentLine, CreditMemoLine, SalesReturnLine, Item.
-- Expected output shape: One row per item group, collection, style family, material, and lifecycle status.
-- Recommended build mode: Either.
-- Interpretation notes: Net sales reduce billed sales by credited returns; net cost reduces shipped standard cost by returned standard cost.
...
WITH billed_sales AS (
    SELECT
        ...
        COALESCE(i.PrimaryMaterial, '(No Material)') AS PrimaryMaterial,
    ...
    GROUP BY
        ...
        COALESCE(i.PrimaryMaterial, '(No Material)'),
...
attribute_keys AS (
    SELECT ItemGroup, CollectionName, StyleFamily, PrimaryMaterial, LifecycleStatus FROM billed_sales
    UNION
    SELECT ItemGroup, CollectionName, StyleFamily, PrimaryMaterial, LifecycleStatus FROM credited_returns
)
SELECT
    ak.ItemGroup,
    ak.CollectionName,
...
LEFT JOIN billed_sales AS bs
   ...
   AND bs.PrimaryMaterial = ak.PrimaryMaterial
...
LEFT JOIN credited_returns AS cr
   ...
   AND cr.PrimaryMaterial = ak.PrimaryMaterial
